...
sc1 = {
    ...
    // camera.position.z = 100;
        ...
        // ";//vec2(abs(sin(vUv.x)),abs(cos(vUv.y)))
         ...
         var fragRT="\
            ...
            void main() {\
                ...
                vec2 oUV2 = vUv+vec2(0.,noise(time*.1+vUv*30.45)*.003);\
                vec4 fat2 = texture2D(fatTexture, oUV);\
                vec4 fat3 = texture2D(fatTexture, oUV2);\
                float offer = (1.5+sin(time*5.))*.5;\
                gl_FragColor = max(\
                    vec4(vec3(ball.x*(1.+sin(time)*.1),ball.y*.9,ball.z*.94)*\
                        vec3(ball.x*(1.+sin(time)*.1),ball.y*.9,ball.z*.94)\
                        ,1.0)\
                    ,fat3*.99);\
            }\
        ";//vec2(abs(sin(vUv.x)),abs(cos(vUv.y)))
        ...
        var fragRT2="\
            ...
            void main() {\
                float offset = texture2D(tDiffuse, vUv).x;\
                vec2 oUV2 = vUv+vec2(0.,noise(time+vUv*3.45)*.001);\
                                vec4 fat3 = texture2D(tDiffuse, oUV2);\
        ...
        sceneRTT.add(lgt);
        sph = sphere(1,50,50);
        // sph = new THREE.Mesh(new THREE.TorusGeometry( .3, .2, 50, 50 ),new THREE.MeshLambertMaterial())
        sceneRTT.add(sph);

        sceneRTT2 = new THREE.Scene();
        // cameraRTT2 = new THREE.OrthographicCamera( -50, 50, 50, -50, 1, 100 );
        cameraRTT2 = new THREE.PerspectiveCamera( 59.5, window.innerWidth / window.innerHeight, 1, 100000 );
         // camera.position.z = 300;
        cameraRTT2.position.z = 76;
        cameraRTT2.rotation.z = 1;
        sceneRTT2.add(pln);
    ...
    draw:function(){
        ...
        scaleMe(sph,3+Math.abs(omouseX)*40)
         cameraRTT2.rotation.z = mouseX+Math.sin(count*.03)*.01;
         cameraRTT2.rotation.y = omouseX*.2;
         cameraRTT2.rotation.x = omouseY*.2;
