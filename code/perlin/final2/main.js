...
canvas1.setup = function() {
  ...
  this.makeSpringyTentacles(7,30);
  f.log(this.pTents);
}
...
canvas1.makeSpringyTentacles = function(num,joints){
  for(var i = 1 ; i < num; i++){
    ...
    this.addTent(this.spTents,joints,'sphere',i,12);
  }
...
canvas1.makeCthulu = function(){
  ...
  for (var p = 0 ; p < 3; p++){
    ...
    for (var i = 0 ; i < num ; i++){
      this.addObject(createParametric(1/(32*2),1/32,sph), 'squishy');
...
canvas1.moveCthulu = function(mouseX,mouseY){
  ...
  //   var b = map_range(i,3,11,0,PI*1.8);
  //   var sweep = msin(b,-.3,.4);
  //   var swoop = mcos(b,-.3,.3);

  //   C = [   o.ball25.matrix[12],
  //           o.ball25.matrix[13]+.05,
  //           o.ball25.matrix[14]
  //       ];  
  
  //   R = [   C[0]+sweep,
  //           C[1]-.4+this.mnoise((time)+i,.1,.2),
  //           C[2]+swoop
  //       ];  
  //   Q = [   C[0]+sweep*.5,
  //           C[1]-.4,
  ...
  //   this.moveTentacle(i,C,R,Q,".8*(.1+(j+1)/25)");
  // }
}
