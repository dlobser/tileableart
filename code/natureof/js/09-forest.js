...
sc1.prototype.init = function() {
	...
	light = new THREE.PointLight( 0x1133ff );
	light.intensity = 1;
	light.distance = 200;
...
sc1.prototype.moveEnv = function(){
	...
	for(thing in this.trees){
		...
		this.treesFull[thing].position.y = Math.sin(this.counter * this.tops[thing].q)*10;
...
sc1.prototype.addEnv = function(){
	...
	var spheregeo = new THREE.CylinderGeometry(0,20,70,4,1);
	...
	var noiseTexture = THREE.ImageUtils.loadTexture('../images/texture/noise.jpg');

	var materialNoise = new THREE.MeshLambertMaterial({
        map: noiseTexture,
		color: 0x33aaff,
     });
	...
	for(var j = 0 ; j < 2 ; j++){
		for(var i = 0 ; i < 50 ; i++){
			...
			mesh2 = new THREE.Mesh( cylgeo2, materialNoise );
	...
	for(var j = 0 ; j < 3 ; j++){
		for(var i = 0 ; i < 400 ; i++){
			...
			mesh = new THREE.Mesh( spheregeo, materialNoise );
			...
			*/
			...
			if(j==0)
				...
				ball.scale = new THREE.Vector3(5,0.1,5);	
			}
	...
	light = new THREE.PointLight( 0xff1100 );
	light.intensity = 10;
	light.distance = 300;
	...
	light = new THREE.PointLight( 0x996600 );
	light.intensity = 5;
	light.distance = 150;
	light.position.set( 0, 0, -300 );
