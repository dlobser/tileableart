...
//		console.log("wtf");
	...
	loader.load( 'models/vertebrae_lorez.obj' );
...
sc1.prototype.checkLoad = function() {
...
sc1.prototype.addGeo = function(){
	...
	this.delayer = document.getElementById('user3');
	this.delay = this.delayer.value || 100;
	...
	var dir = new THREE.Vector3(.1,.5,-.4);
		...
		while(totalJoints > 550 || totalJoints < -500){
			...
			while(numJoints > 250 || numJoints < 20){
				...
				your_object.jScale[1].y = rand(8,22);
				your_object.jScale[1].x = rand(1,6);
			...
			while(numJoints2 > 900 || numJoints2 < 200){
...
sc1.prototype.moveThings = function(){
	...
	this.poofArraySize = document.getElementById('user2');
	


	var poof = this.poofArraySize.value || 8;

	
	var avg = this.averager.avg(thisKey-48,20,0);
	var avg2 = this.averager.avg(thisKey-48,poof,1);
	var avg3 = this.averager.avg(thisKey-48,3,2);
	//console.log(avg2);
...
//		console.log(this.scene.fog.density);
	...
	*/
	...
	if(avg2>8 && particleIn && readyToBurst && this.delay < 2){
		helpGeo = true;
		readyToBurst = false;
		built = true;
	...
	if(count%2==0){
	this.scaleBones(((avg3+48)*.17)-7,things[0].branches);
	this.scaleBones(((avg3+48)*.17)-7,things[1].branches);
	}
