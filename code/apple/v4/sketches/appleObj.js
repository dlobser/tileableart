...
rainbow.prototype.makeStem = function(redraw){
	...
	if(!drawer){
		...
		var sp = new THREE.Mesh(new THREE.TubeGeometry2(new THREE.SplineCurve3(stemPoints,12),100,1.1),new THREE.MeshLambertMaterial({color:0x332211}))
...
rainbow.prototype.makeLeaf = function(params){
	...
	for(var j = 0 ; j < 6 ; j++){
		...
		for(var i = 0 ; i < len*detail ; i++){
			...
			turtle.position = pos;
			turtle.lookAt(dir);
			var v = pCurve.getPointAt(0);
			var u = pCurve.getPointAt(1);
