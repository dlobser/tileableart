...
colorCanvas = new makeSpectrum(e, "myCanvas3");
// colors.update();
...
function makeSpectrum(x,canvas){

	var c = document.getElementById(canvas);
	var ctx = c.getContext("2d");
	var colorPicker = [{x:0,y:0},{x:149,y:149}];
	

	ctx.colors = [];
	

	lerpColors(ctx,colorPicker,{x:0,y:0});
	...
	c.addEventListener('mousemove', function (evt) {
		...
		if (drawLine) {
			...
			lerpColors(ctx,colorPicker,mousePos);
	...
	this.update = function(go) {
		...
		drawSpectrum(ctx,colorPicker);

		// requestAnimationFrame(this.update);
		// console.log(ctx.getImageData(0,0,150,150));
	}
}

function lerpColors(ctx,colorPicker,mousePos){
	ctx.colors = [];
	for(var i = 0 ; i < colorPicker.length ; i++){
		// console.log(Math.abs(colorPicker[0].x ));
		if( dist(mousePos,colorPicker[i])<20 || moveCtrl == i){
			colorPicker[i].x = mousePos.x;
			colorPicker[i].y = mousePos.y;
			moveCtrl = i;
			i=colorPicker.length;

		}
	}
	for(var i = 0 ; i <= ctx.steps ; i++){
		// console.log(colorPicker[0]);
		var col = [];
		col.push(Math.floor(mLerp(colorPicker[0].x,colorPicker[1].x,i/ctx.steps)));
		col.push(Math.floor(mLerp(colorPicker[0].y,colorPicker[1].y,i/ctx.steps)));
		ctx.colors.push(col);
		// console.log(col);
	}

}
//magnet:?xt=urn:btih:7AFC023FBE8059A92DCEB41EF8EC1C516E16D39D&dn=top+gear+vietnam+special+sideiron+avi&tr=udp%3A%2F%2Ftracker.1337x.org%3A80%2Fannounce&tr=udp%3A%2F%2Fopen.demonii.com%3A1337
function drawSpectrum(ctx,s){


	// console.log('h')
	var id = ctx.createImageData(150,150); // only do this once per page
	var d  = id.data;                        // only do this once per page
	
	var dat = [];
	// console.log(ctx.colors);

	var rand = 0;//Math.random()*30;
	// 
	if(typeof data !== 'undefined')
		rand=data.var7;
	else
	...
	for(var i = 0 ; i < 150 ; i++){
		for(var j = 0 ; j < 150 ; j++){
			...
			var oi1 = i+.6+noise(rand+j*  freq,.5+j*freq,i*freq)*60.5;
			var oi2 = i+.6+noise(2+rand+j*freq,.5+j*freq,i*freq)*60.5;
			var oi3 = i+.6+noise(1+rand+j*freq,.5+j*freq,i*freq)*60.5;

			var r = (2+Math.sin(oi1*Math.PI*2/150))/3;
			var g = ((2+Math.sin((oi2*Math.PI*2)/150))/3);
			var b = (2+ Math.sin((oi3*Math.PI*2)/150))/3;//(1+Math.cos(((Math.PI*2/3)*2)+(i*Math.PI*2)/150))/2;


			
			// var s = ((1+Math.cos((j/150)*Math.PI))/2);
			var v = 1;//((1+Math.cos((j/300)*Math.PI))/2);
			dat.push([Math.sin(r)*v*255,Math.sin(g)*v*255,Math.sin(b)*v*255,255]);
	...
	if(ctx.colors){
		for(var k = 0 ; k < ctx.colors.length ; k++){
			var i = Math.floor(ctx.colors[k][0])
			var j = Math.floor(ctx.colors[k][1])
			// var val = dat[(ctx.colors[i][0])+(ctx.colors[i][1]*150)];
					// console.log(val);
			var xy = i+(j*150);
			colors.push(dat[xy]);
		}
	...
	// 	for(var i = 0 ; i < ctx.colors.length ; i++){
	// 		var val = dat[(ctx.colors[i][0])+(ctx.colors[i][1]*150)];
	// 				// console.log(val);
