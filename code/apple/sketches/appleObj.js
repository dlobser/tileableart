...
rainbow.prototype.extrudeSpine = function(params){
	...
	for(var j = 0 ; j < 2 ; j++){
		...
		for(var i = 0 ; i < this.spineCurve[j].length ; i++){
			...
			if(j==1 && i==this.spineCurve[j].length-1 && drawStem){
				var sp = new THREE.Mesh(new THREE.TubeGeometry2(new THREE.SplineCurve3([new THREE.Vector3(0,0,0),new THREE.Vector3(0,3,.5),new THREE.Vector3(0,6,1.5)],12)),new THREE.MeshLambertMaterial({color:0x332211}))
				sp.stem=true;
			}
			else{
