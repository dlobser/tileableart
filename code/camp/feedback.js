...
var mouseX,mouseY,omouseX,omouseY;
var count = 0;
...
sc1 = {
	...
	setup:function(){
		...
		var fragRT="\
			...
			void main() {\
				vec4 offset = texture2D(tDiffuse, vUv);\
				vec4 fat = texture2D(fatTexture, vUv);\
				vec4 col = texture2D(tDiffuse, vUv);\
				gl_FragColor = max(offset,fat);\
			}\
		";
		...
		rtTexture2 = new THREE.WebGLRenderTarget( 512,512, { minFilter: THREE.LinearFilter, magFilter: THREE.LinearFilter, format: THREE.RGBFormat } );
		...
		var vertRT2 = "\
			...
			void main() {\
				vUv = uv;\
				float offset = texture2D(tDiffuse, vUv).x;\
				float noisex = noise(vec3(vUv*10.,(time*.1)))*.3;\
				float noisey = noise(vec3(vUv*11.,(time*.1)))*.3;\
				gl_Position = projectionMatrix * modelViewMatrix * vec4( vec3(position.x+noisex,position.y+noisey,position.z + offset*10.00), 1.0 );\
			}\
		";
		...
		//concat noise to the shaders
		
		var nsr = noise+fragRT2;
		var vnsr = noise+vertRT2

		var materialScreen2 = new THREE.ShaderMaterial( {
			uniforms: { tDiffuse: { type: "t", value: rtTexture2 }, time:{type:"f",value:0}  },
			vertexShader: vnsr,
			fragmentShader: nsr,
			depthWrite: false
		} );
		...
		var geo = new THREE.PlaneGeometry(100,100,300,300);

		// for(var i = 0 ; i < geo.vertices.length ; i++){
		// 	geo.vertices[i].x+=Math.sin(i*.04)*.1;
		// 	geo.vertices[i].y+=Math.cos(i*.04)*.1;

		// 	geo.verticesNeedUpdate = true;
		// }
		
		pln = new THREE.Mesh(geo,materialScreen);
		pln2 = new THREE.Mesh(geo,materialScreen2);
		pln3 = new THREE.Mesh(geo,materialScreen2);
		...
		sceneRTT.add(lgt);
		spheres = [];
		sph = new THREE.Vector3(0,0,0);
		oldSpherePosition = new THREE.Vector3(0,0,0);

		for(var i = 0 ; i < 30 ; i++){
			spheres.push(sphere(2,15,15));
			sceneRTT.add(spheres[i]);
		}
		...
		cameraRTT2.position.z = 66;
		// cameraRTT2.rotation.z = Math.PI/3.00133;
		sceneRTT2.add(pln);
	...
	draw:function(){
		...
		count++;
		// cameraRTT2.rotation.z = omouseX;

		// console.log(pln3);

		scaleMe(sph,3+Math.abs(omouseX)*40)
		...
		for(var i = 0 ; i < spheres.length ; i++){
			var pos = sph.clone();
			spheres[i].material.color.setHSL(Math.random(),.8,.8);//=new THREE.Color(1,1,1);

			pos.lerp(oldSpherePosition,i/30);
			spheres[i].position.x = pos.x+Math.random();//pos.lerp(oldSpherePosition,i/10);
			spheres[i].position.y = pos.y+Math.random();
			spheres[i].position.z = pos.z;
		}

		pln.material.uniforms["time"].value = count*.023;
		pln3.material.uniforms["time"].value = count*.043;

		//rendering to render targets happens here, rendering the actual plane for display happens elsewhere

		if(count%2==0){
			oldSpherePosition = sph.clone();

			//renders scene with RTT2 plane
			renderer.render( sceneRTT3, cameraRTT3, fatTexture, true );
			//render the scene with the ball to rtTexture
			renderer.render( sceneRTT, cameraRTT, rtTexture, true );
		}
		else{
			//this scene renders the plane with RTT texture
...
function scaleMe(obj,s){
	obj.scale = new THREE.Vector3(s,s,s);
}
