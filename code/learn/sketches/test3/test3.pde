...
void draw(){
 ...
 // background(255);
  ...
  for(int i = 0 ; i < boxes.size() ; i++){
   ...
   text(note + " " + frameRate,width/2,height/2);
...
class Rect{
  ...
  Rect(float ux, float uy, float lx, float ly, int sx, int sy, int amount){
   ...
   // println(uX + " " + lX);
    ...
    box = new wRect(uX,uY,lX,lY,1,0,grid[0][2]);
    box.ID = ID;
...
//        break;
  ...
  void display(){  
    ...
    if(amt==0){
      ...
      box.display(-5,5,myColor);
      if(ID==note){//>note && ID<note+noter){
        // stroke(255,0,0,128);
        fill(255,255,0,random(255));
        //rect(uX,uY,lX,lY); 
        //wRect(uX,uY,lX,lY,note,1,grid[0][2]); 
        fill(255,255);
        float plunker;
        plunker=min(dist(uX,uY,lX,uY),dist(uX,uY,uX,lY));
        box.plunk(plunker*.02);
...
//    }
  ...
  float getPitch(){
    return map(dist(uX,uY,lX,lY),0,width,80,15);
  }
...
class wRect{
  ...
  //has it been plunked - don't plunk it again!
  boolean plunkered;
  
  wRect(float ux, float uy, float lx, float ly, float go, float plunked, float thick){
    grow=0;
    uX = ux;
    uY = uy;
    lX = lx;
  ...
  void display(float low, float high, color myColor){
...
//    fill(0,255);
//      text(abs(plunk),lerp(uX,lX,.5)+10,lerp(uY,lY,.5));
