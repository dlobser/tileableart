...
void draw(){
  ...
  if(note==0){
    ...
    rectMode(CORNERS);
    frameRate(noter);
    amount = int(random(5,12));//int(random(5,map(sin(time*.001),-1,1,6,12)));
    //background(255);
  ...
  //  sc.playChord(pitches, 80, 4);
    ...
    frameRate(map(sin(time*.1),-1,1,2,8));//random(2,10)));
    brecht.display();
...
//      line(lX,uY,uX,lY);
  ...
  void split(int minus){
    ...
    IDer++;
    ID = IDer;
            pitches = expand(pitches,map(uX-lX/uY/lY,0,2000,80,15));
