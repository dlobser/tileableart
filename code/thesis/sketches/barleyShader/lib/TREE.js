		...
		for (var j = 0; j < array[i].length; j++) {
		 	...
		 	for (var k = 0; k < process.length; k++) {
		 		if(accelerated){
					...
					func(this.bones[this.Move(process[k],this.findInMirrorArray,this.mirrorJointArray)],array[i].args);
		 		}
		 		else{
...
TREE.prototype.transform = function (obj,args){
	...
	if(args){
		...
		nMult = nFreq = nOff = nFract = offScale = offScaleOff = offsetter4 = 0;
		GPU = rotator = false;
	}
